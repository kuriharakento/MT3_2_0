...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		Vector3 project = Project(Sub(point, segment.origin), segment.diff);
		Vector3 closetPoint = ClosestPoint(project, segment);

		Sphere pointSphere{ point,0.01f };
		Sphere closetPointSphere{ closetPoint,0.01f };
		...
		DrawSphere(closetPointSphere, viewProjectionMatrix, viewportMatrix, BLACK);
...
Vector3 ClosestPoint(const Vector3& point, const Segment& segment)
{
	Vector3 proja = Project(Sub(point, segment.origin), Sub(Add(segment.origin, segment.diff), segment.origin));
